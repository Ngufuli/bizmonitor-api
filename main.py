...
def add_member(business_id: int, data: schemas.BusinessMemberAdd, current_user: models.User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    return {"id": member.id, "user_id": member.user_id, "business_id": member.business_id, "role": str(member.role)}

@app.patch("/businesses/{business_id}/members/{user_id}", tags=["Businesses"])
def update_member_role(business_id: int, user_id: int, data: schemas.BusinessMemberAdd, current_user: models.User = Depends(require_admin), db: Session = Depends(get_db)):
    member = crud.add_member(db, business_id, data, added_by_id=current_user.id)
    if not member:
